    ...
    for block_i, (model_data, quantizer) in enumerate(blocks):
        ...
        for row in grid_layout:
            for _, data, uni, var in row:
                ...
                all_group_recalls.extend(uni)
                all_group_recalls.extend(var)
        ...
        for r in range(2):
            for c in range(3):
                ...
                ax_twin = ax.twinx()
                l2 = ax_twin.plot(bpd_array, uni, marker='^', color='#999999', linestyle='--', linewidth=2, markersize=6, alpha=0.6, label='Uniform Recall')
                l3 = ax_twin.plot(bpd_array, var, marker='v', color='#444444', linestyle='--', linewidth=2, markersize=6, alpha=0.6, label='Variable Recall')
